...
int main(){
    ...
    printf("%d\n", emufs_create(dir2, "file1", 0));     // 1
    printf("%d\n", emufs_create(dir2, "file2", 0));     // 1
    printf("%d\n", emufs_create(dir2, "file3", 0));     // 1
    printf("%d\n", emufs_create(dir2, "file4", 0));     // 1

    int fd1 = open_file(dir2,"file1");  // 0
    // foo(fd1);
    char str1[] = "!-----------------------64 Bytes of Data-----------------------!!-----------------------64 Bytes of Data-----------------------!!-----------------------64 Bytes of Data-----------------------!!-----------------------64 Bytes of Data-----------------------!";
    emufs_write(fd1, str1, 64*4);       // 1
    emufs_write(fd1, str1, 64*4);       // 1
    char buf1[256 * 2 + 1];
    buf1[256 * 2] = '\0';
    emufs_seek(fd1, -256*2);            // 1
    emufs_read(fd1, buf1, 256*2);       // 1
    printf("Data: %s\n", buf1);
    emufs_close(fd1, 0);
    fsdump(mnt2);
